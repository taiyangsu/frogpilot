...
class LongitudinalPlanner:
  ...
  def update(self, sm):
    ...
    if self.conditional_experimental_mode and sm['controlsState'].enabled:
      ...
      if status_bar != self.previous_status_bar:
        ...
        put_int_nonblocking("ConditionalStatus", status_bar)
  ...
  def check_conditions(self, sm, v_ego, v_lead):
    ...
    stop_light_detected = self.stop_sign_and_light(carstate, lead, modeldata, radarstate, v_ego, v_lead) and not standstill
  ...
  def stop_sign_and_light(self, carstate, lead, modeldata, radarstate, v_ego, v_lead):
    if self.stop_lights and abs(carstate.steeringAngleDeg) <= 60 or self.stop_light_count >= THRESHOLD:
      ...
      if not lead or (lead and not (self.previous_lead_speed >= v_lead or radarstate.leadOne.dRel <= 10 or v_lead <= 1)):

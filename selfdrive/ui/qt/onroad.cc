...
void OnroadWindow::updateState(const UIState &s) {
  ...
  // Calculate FPS
  if (developerUI) {
    constexpr double minAllowedFPS = 1.0;
    constexpr double maxAllowedFPS = 99.9;

    qint64 elapsedMillis = fpsTimer.restart();
    double fps = 1000.0 / elapsedMillis;
    lastFPS = 0.9 * lastFPS + 0.1 * fps;
...
void AnnotatedCameraWidget::updateState(const UIState &s) {
  ...
  vtscOffset = floor(0.1 * sm["longitudinalPlan"].getLongitudinalPlan().getVtscOffset() * (s.scene.is_metric ? MS_TO_KPH : MS_TO_MPH) + 0.9 * vtscOffset);

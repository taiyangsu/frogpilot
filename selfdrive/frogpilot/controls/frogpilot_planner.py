...
class FrogPilotPlanner:
  ...
  def update(self, carState, controlsState, frogpilotCarControl, frogpilotNavigation, liveLocationKalman, modelData, radarState):
    ...
    if FrogPilotToggles.acceleration_profile == 1 or self.params_memory.get_bool("EcoGearOn"):
      self.max_accel = get_max_accel_eco(v_ego)
    elif FrogPilotToggles.acceleration_profile in {2, 3} or self.params_memory.get_bool("SportGearOn"):
    ...
    if (FrogPilotToggles.deceleration_profile == 1 or self.params_memory.get_bool("EcoGearOn")) and not v_cruise_changed:
      self.min_accel = get_min_accel_eco(v_ego)
    elif (FrogPilotToggles.deceleration_profile == 2 or self.params_memory.get_bool("SportGearOn")) and not v_cruise_changed:
    ...
    if FrogPilotToggles.conditional_experimental_mode and self.CP.openpilotLongitudinalControl or FrogPilotToggles.green_light_alert:
  ...
  def update_follow_values(self, jerk, lead_one, t_follow, trafficModeActive, v_ego, v_lead):
    ...
    if FrogPilotToggles.aggressive_acceleration:
    ...
    if FrogPilotToggles.smoother_braking:
      distance_factor = np.maximum(lead_distance - (v_lead * t_follow), 1)
      far_lead_offset = max(lead_distance - (v_ego * t_follow) - stopping_distance, 0) if FrogPilotToggles.smoother_braking_far_lead and v_lead < v_ego else 0

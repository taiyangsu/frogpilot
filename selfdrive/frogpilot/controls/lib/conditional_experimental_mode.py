...
class ConditionalExperimentalMode:
  ...
  def update(self, carState, enabled, frogpilotNavigation, lead, modelData, road_curvature, t_follow, v_ego):
    ...
    if FrogPilotToggles.experimental_mode_via_press and enabled:
  ...
  def check_conditions(self, carState, frogpilotNavigation, lead, modelData, standstill, v_ego):
    ...
    if FrogPilotToggles.conditional_navigation and modelData.navEnabled and (frogpilotNavigation.approachingIntersection or frogpilotNavigation.approachingTurn) and (FrogPilotToggles.conditional_navigation_lead or not self.lead_detected):
    ...
    if (not self.lead_detected and v_ego <= FrogPilotToggles.conditional_limit) or (self.lead_detected and v_ego <= FrogPilotToggles.conditional_limit_lead):
      self.status_value = 10 if self.lead_detected else 11
      return True

    if FrogPilotToggles.conditional_slower_lead and self.slower_lead_detected:
      self.status_value = 12
      return True

    if FrogPilotToggles.conditional_signal and v_ego <= CITY_SPEED_LIMIT and (carState.leftBlinker or carState.rightBlinker):
      self.status_value = 13
      return True

    if FrogPilotToggles.conditional_curves and self.curve_detected:
      self.status_value = 14
      return True

    if FrogPilotToggles.conditional_stop_lights and self.red_light_detected:
  ...
  def stop_sign_and_light(self, lead_distance, modelData, standstill, v_ego, v_lead):
    lead_check = FrogPilotToggles.conditional_stop_lights_lead or not self.lead_slowing_down(lead_distance, v_ego, v_lead) or standstill

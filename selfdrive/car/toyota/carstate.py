...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam, frogpilot_variables):
    ...
    cp_acc = cp_cam if (self.CP.carFingerprint in (TSS2_CAR - RADAR_ACC_CAR) or bool(self.CP.flags & FrogPilotToyotaFlags.DSU_BYPASS.value)) else cp

    if (self.CP.flags & FrogPilotToyotaFlags.DSU_BYPASS.value) or (self.CP.carFingerprint in TSS2_CAR and not self.CP.flags & ToyotaFlags.DISABLE_RADAR.value):
    ...
    if self.CP.flags & FrogPilotToyotaFlags.ZSS and self.zss_threshold_count < ZSS_THRESHOLD_COUNT:
  ...
  def get_can_parser(CP):
    ...
    if CP.carFingerprint not in (TSS2_CAR - RADAR_ACC_CAR) and not CP.enableDsu and not (CP.flags & ToyotaFlags.DISABLE_RADAR.value or CP.flags & FrogPilotToyotaFlags.DSU_BYPASS.value):
      messages += [
        ("PRE_COLLISION", 33),
      ]
  ...
  def get_cam_can_parser(CP):
    ...
    if CP.flags & FrogPilotToyotaFlags.DSU_BYPASS.value or CP.carFingerprint in (TSS2_CAR - RADAR_ACC_CAR):
      messages += [
        ("PRE_COLLISION", 33),
        ("ACC_CONTROL", 33),
        ("PCS_HUD", 1),
